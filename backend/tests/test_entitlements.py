...
def test_pricing_version_2026_1_is_pinned_and_unaffected_by_new_modules():
    ...
    # Guncel surum 2026.4 (ai_interaction_heatmap ucreti eklendi; 2026.3 ai_report
    # ucreti). 2026.1 pinlenmis kalir; yeni gelismis moduller yalnizca 2026.2+.
    assert CURRENT_PRICING_VERSION == "2026.4"
    ...
    assert old.interaction_heatmap_chip_cost == 0  # heatmap eski surumde yok
    with pytest.raises(ValueError):
        old.module_cost("network_device_test")

    current = get_pricing_config()
    assert current.version == "2026.4"
    assert current.ai_report_chip_cost == 50
    assert current.interaction_heatmap_chip_cost == 30
