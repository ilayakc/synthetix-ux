...
def test_catalog_lists_seven_active_modules_with_full_metadata(client, monkeypatch):
    # Hazirlik bayraklari (ai_report_enabled / ai_interaction_heatmap_enabled)
    # ortama gore degisebildigi icin (dev .env AI_REPORT_ENABLED=true olabilir),
    # bu test "her iki AI modulu de hazir DEGIL" varsayilan sozlesmesini
    # DETERMINISTIK dogrular: iki bayrak da acikca False'a sabitlenir.
    ...
    # Guncel katalog surumu 2026.4 (ai_report + ai_interaction_heatmap eklendi),
    # ama her ikisi de hazirlik bayraklari kapali oldugundan katalog yanitindan
    # GIZLENIR -> gorunur modul sayisi yine 7.
    assert body["catalog_version"] == "2026.4"
    assert len(body["modules"]) == 7
    assert "ai_report" not in {m["key"] for m in body["modules"]}
    assert "ai_interaction_heatmap" not in {m["key"] for m in body["modules"]}
    assert "ai_interaction_heatmap" not in {m["key"] for m in body["modules"]}
...
def test_quote_prices_new_advanced_modules(client):
    ...
    assert response.status_code == 200, response.text
    body = response.json()
    assert body["pricing_version"] == "2026.4"
