...
class HeuristicRulesConfig:
    ...
    # --- Kampanya CTA tiklama olasiligi (app.engine.advanced_modules) ---
    # Taban tiklama olasiligi (sirada ilk, ust ekranda, yeterli kontrastlı CTA icin).
    cta_base_click_probability: float = 0.30
    # Her ek CTA sirasi (rank) basina azalan ilgi cezasi.
    cta_rank_penalty: float = 0.045
    # CTA ilk ekranda ise sabit bonus, degilse sabit ceza.
    cta_above_fold_bonus: float = 0.05
    cta_below_fold_penalty: float = 0.07
    # Sayfa ortalama kontrasti WCAG AA esiginin altindaysa sabit ceza.
    cta_low_contrast_penalty: float = 0.06
    # Mesaj netligi bulgusu esikleri.
    cta_too_many_ctas_threshold: int = 3

    # --- Sentetik dikkat tahmini bolge agirliklari (app.engine.advanced_modules) ---
    # Her bolge icin taban (normalize edilmeden once) agirlik.
    attention_base_weight_top_nav: float = 12.0
    attention_base_weight_hero: float = 22.0
    attention_base_weight_primary_cta: float = 18.0
    attention_base_weight_body: float = 30.0
    attention_base_weight_footer: float = 6.0
    # Birincil CTA ilk ekrandaysa hero+CTA bolgelerine eklenen bonus.
    attention_above_fold_bonus: float = 8.0
    # Basina eklenen kucuk katki (baslik sayisi basina, ust siniri var).
    attention_heading_bonus_per_heading: float = 0.8
    attention_heading_bonus_max: float = 6.0
    # Govde metni agirligina kelime sayisina gore eklenen katki (ust sinirli).
    attention_body_bonus_per_100_words: float = 0.6
    attention_body_bonus_max: float = 10.0
    # Seed'li kucuk varyasyonun (jitter) genisligi (taban agirligin orani).
    attention_jitter_ratio: float = 0.08
